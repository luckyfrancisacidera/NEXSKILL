...
namespace SkillSense.Application.Interfaces.Jobseeker
{
    public interface IJobSeekerService
    {
        ...
        Task<PagedResult<JobSeekerApplicationResponse>> GetMyApplicationsAsync(Guid userId, int pageNumber, int pageSize, string? search, string? status, DateTime? startDate, DateTime? endDate, bool archivedOnly = false, CancellationToken ct = default);
        ...
        Task ArchiveApplicationHistoryAsync(Guid userId, Guid applicationId, CancellationToken ct = default);
        Task UnarchiveApplicationHistoryAsync(Guid userId, Guid applicationId, CancellationToken ct = default);
        Task DeleteApplicationHistoryAsync(Guid userId, Guid applicationId, CancellationToken ct = default);
    }
}
