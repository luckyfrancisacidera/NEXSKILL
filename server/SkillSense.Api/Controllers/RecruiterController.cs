    ...
    ILogger<RecruiterController> logger) : ControllerBase
{
    ...
    private (Guid UserId, Guid CompanyId) GetRecruiterContext()
        => (CurrentUserId, RequireCompanyId());

    private (Guid UserId, Guid CompanyId, Guid? RecruiterProfileId) GetRecruiterAuditContext()
        => (CurrentUserId, RequireCompanyId(), CurrentUserContext.GetActiveRecruiterProfileId(HttpContext));

    // Loads profile.
    [HttpGet("profile")]
    public async Task<ActionResult<RecruiterProfileResponse>> GetProfile(CancellationToken ct)
        => Ok(await recruiterService.GetProfileAsync(CurrentUserId, ct));
    ...
    public async Task<ActionResult<RecruiterProfileResponse>> UpdateProfile([FromBody] RecruiterProfileRequest request, CancellationToken ct)
        => Ok(await recruiterService.UpsertProfileAsync(CurrentUserId, request, ct));

    // Creates job.
    [HttpPost("jobs")]
    public async Task<ActionResult<JobListItemResponse>> CreateJob([FromBody] CreateJobRequest request, CancellationToken ct)
    {
        var (userId, companyId, recruiterProfileId) = GetRecruiterAuditContext();
    ...
    // Loads applicant resume download.
    ...
    // Downloads applicant resume file.
