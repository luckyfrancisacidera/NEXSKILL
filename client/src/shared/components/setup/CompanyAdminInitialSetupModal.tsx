...
export const CompanyAdminInitialSetupModal = ({
  onCompleted,
}: CompanyAdminInitialSetupModalProps) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!companyName.trim() || !adminName.trim()) {
      setError("Company name and admin name are required.");
      return;
    }
    ...
    setError(null);
    try {
      await http.post("/api/account/setup/company-admin", {
        companyName,
        companyEmail,
        location,
        adminName,
    ...
    <ModalOverlay onClose={() => undefined}>
      <div className="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-zinc-200 dark:bg-zinc-950 dark:ring-zinc-800">
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Set up your company details so you can manage recruiters, jobs, and
          interviews for your organization.
        </p>
