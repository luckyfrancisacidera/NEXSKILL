...
import { SortableHeader } from "@shared/components/data-display/data-table/SortableHeader";
import type {
  DataTableColumn,
  DataTableRowConfig,
  DataTableSortState,
  DataTableSortType,
} from "@shared/components/data-display/data-table/table-types";
...
<div className="block w-full min-w-0 max-w-full overflow-x-auto overflow-y-hidden overscroll-x-contain">
      ...
      >
        {toolbar ? (
          <div className="w-full min-w-0 max-w-full border-b border-zinc-200 px-3 py-3 dark:border-zinc-800 sm:px-4 sm:py-4">
            {toolbar}
          </div>
        ) : null}
        {/* Table scroll is isolated here instead of letting the page overflow. */}
        <div className="w-full min-w-0 max-w-full overflow-x-auto overflow-y-hidden overscroll-x-contain">
          <table className={cn("w-full border-collapse table-auto", tableClassName)}>
                ...
                {columns.map((column) => {
                  ...
                  return (
                    <th
                      ...
                      className={cn(
                        "min-w-0 px-2.5 py-2.5 align-middle text-[9px] font-medium uppercase leading-4 text-zinc-500 first:pl-3 last:pr-3 dark:text-zinc-400 min-[420px]:text-[10px] sm:px-3 sm:py-3 sm:text-[11px] sm:first:pl-4 sm:last:pr-4",
                        alignClassName,
                ...
                ? Array.from({ length: loadingRowCount }, (_, rowIndex) => (
                  ...
                  >
                    {columns.map((column) => (
                        <td
                          key={column.id}
                          className="min-w-0 border-b border-zinc-200 px-2.5 py-2.5 align-middle first:pl-3 last:pr-3 dark:border-zinc-800 sm:px-3 sm:py-3 sm:first:pl-4 sm:last:pr-4"
                        >
                        ...
                        {columns.map((column) => {
                          ...
                          return (
                            <td
                              key={column.id}
                              className={cn(
                                "min-w-0 border-b border-zinc-200 px-2.5 py-2.5 align-middle text-[11px] leading-[1.125rem] text-zinc-700 first:pl-3 last:pr-3 dark:border-zinc-800 dark:text-zinc-300 min-[420px]:text-[12px] sm:px-3 sm:py-3 sm:leading-5 sm:first:pl-4 sm:last:pr-4",
                                alignClassName,
                                column.widthClassName,
                                cellClassName,
                              )}
