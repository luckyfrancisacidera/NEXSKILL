...
const getSubscriptionProgress = (
  ...
  if (!startsAtUtc || !endsAtUtc) return 0;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
          Monitor plan status, company-wide usage, and remaining trial or billing time.
        </p>
      </div>
      ...
      <Card className="space-y-4 bg-zinc-950 text-zinc-100 dark:bg-zinc-100 dark:text-zinc-900">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.25em] text-zinc-400 dark:text-zinc-600">
              {data.isTrial ? "Free Trial" : "Current Plan"}
            </p>
            <h2 className="mt-2 text-3xl font-semibold">{data.planName}</h2>
            <p className="mt-2 text-sm text-zinc-300 dark:text-zinc-700">
              {data.isExpired ? "Expired" : `${data.daysRemaining} days left`}
            </p>
          </div>
          ...
          <div
            className={`rounded-full px-3 py-1 text-sm ${
              data.isExpired
                ? "bg-red-500/15 text-red-200 dark:bg-red-500/10 dark:text-red-700"
                : "bg-white/10 text-zinc-100 dark:bg-zinc-900/10 dark:text-zinc-900"
            }`}
          >
            {data.status}
          </div>
...
<Progress
  value={getSubscriptionProgress(data.startsAtUtc, data.endsAtUtc, data.isExpired)}
  variant="time"
/>

        <p className="text-sm text-zinc-300 dark:text-zinc-700">
          {data.restrictionMessage ?? "Your company subscription is active."}
        </p>
      </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="space-y-3">
          ...
          <p className="text-3xl font-semibold text-zinc-900 dark:text-zinc-100">
            {data.activeJobPostsUsed}
            {data.activeJobPostsMax ? ` / ${data.activeJobPostsMax}` : ""}
          </p>
          <Progress value={usagePercent(data.activeJobPostsUsed, data.activeJobPostsMax)} variant="usage" />
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            {data.usageSharedNoteJobPosts}
          </p>
        </Card>
        ...
        <Card className="space-y-3">
          ...
          <p className="text-3xl font-semibold text-zinc-900 dark:text-zinc-100">
            {data.screeningsUsed}
            {data.screeningsMax ? ` / ${data.screeningsMax}` : ""}
          </p>
          <Progress value={usagePercent(data.screeningsUsed, data.screeningsMax)} variant="usage" />
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            {data.usageSharedNoteScreenings}
          </p>
        </Card>
      </div>
      ...
      <Card className="space-y-3">
        <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
          {data.isTrial ? "Upgrade Path" : data.isExpired ? "Renew Access" : "Plan Notes"}
        </p>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          {data.isTrial
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {data.isExpired ? (
            <a
              href="/company-account-request"
              className="rounded-2xl border border-zinc-300 px-4 py-3 text-sm font-medium text-zinc-700 dark:border-zinc-700 dark:text-zinc-300"
            >
              Renew Access
            </a>
          ) : null}
