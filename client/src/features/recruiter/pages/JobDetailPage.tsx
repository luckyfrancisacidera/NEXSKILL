...
import type { RecruiterJobDetailLoaderData } from '@features/recruiter/types';
...
export const JobDetailPage = () => {
  ...
  useEffect(() => {
    ...
    if (toast === 'created') {
      showToast({ title: 'Job created successfully', description: `${job.title} is ready for recruiter workflows.`, tone: 'success' });
    }

    if (toast === 'updated') {
      showToast({ title: 'Job updated successfully', description: `${job.title} was updated.`, tone: 'success' });
    }
  ...
  const deleteJob = async () => {
    try {
      ...
      showToast({ title: 'Job deleted successfully', description: `${job.title} was removed from your job posts.`, tone: 'success' });
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-5">
          <header className="space-y-4 border-b border-zinc-200 pb-4">
            ...
            <div className="flex flex-wrap gap-2">
              <span className={`rounded-lg border px-3 py-1 text-sm font-medium ${statusAccent.className}`}>{statusAccent.label}</span>
              ...
              <span className="rounded-lg border border-zinc-200 bg-zinc-100 px-3 py-1 text-sm font-medium text-zinc-800">{formatCurrencyAmount(job.salary_min_per_annum, job.currency)} - {formatCurrencyAmount(job.salary_max_per_annum, job.currency)} / year</span>
              <span className="rounded-lg border border-zinc-200 bg-zinc-100 px-3 py-1 text-sm text-zinc-700">Vacancies: {job.remaining_vacancies ?? 0} / {job.number_of_vacancies ?? 0}</span>
            </div>
          </header>

          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_260px]">
            <div className="space-y-4">
              <DetailBlock title="About the Role"><p className="whitespace-pre-wrap leading-7 text-zinc-700">{job.description || 'No description provided.'}</p></DetailBlock>
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                <DetailBlock title="Required Skills">
                  <div className="flex flex-wrap gap-2">
                    {requiredSkills.length > 0 ? requiredSkills.map((skill) => <span key={skill} className="rounded-full border border-zinc-300 bg-zinc-100 px-3 py-1 text-xs font-medium text-zinc-700">{skill}</span>) : <p className="text-sm text-zinc-500">No required skills listed.</p>}
                  </div>
                </DetailBlock>
                <DetailBlock title="Preferred Skills">
                  <div className="flex flex-wrap gap-2">
                    {preferredSkills.length > 0 ? preferredSkills.map((skill) => <span key={skill} className="rounded-lg border border-zinc-300 bg-zinc-100 px-3 py-1 text-sm text-zinc-700">{skill}</span>) : <p className="text-sm text-zinc-500">No preferred skills listed.</p>}
                  </div>
                </DetailBlock>
              </div>
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                ...
                <DetailBlock title="Work Details">
                  <ul className="space-y-2 text-zinc-700">
                    <li><span className="font-medium text-zinc-900">Schedule:</span> {job.schedule || 'Not specified'}</li>
                    <li><span className="font-medium text-zinc-900">Work Setup:</span> {job.work_setup || 'Not specified'}</li>
                    <li><span className="font-medium text-zinc-900">Location:</span> {job.location || 'Not specified'}</li>
                  </ul>
                </DetailBlock>
              </div>
              ...
              <DetailBlock title="Compensation & Benefits">
                <div className="flex flex-wrap items-center gap-2 text-zinc-700">
                  <span className="text-xl font-bold text-zinc-900">{formatCurrencyAmount(job.salary_min_per_annum, job.currency)} - {formatCurrencyAmount(job.salary_max_per_annum, job.currency)}<span className="text-base font-medium text-zinc-600"> / year</span></span>
                  {benefits.length > 0 ? benefits.map((benefit) => <span key={benefit} className="rounded-lg border border-zinc-300 bg-zinc-100 px-3 py-1 text-sm">{benefit}</span>) : null}
                </div>
              </DetailBlock>
            </div>

            <aside className="space-y-3 lg:sticky lg:top-4 lg:self-start">
              <Link to={`/recruiter/job-posts/${job.id}/edit`} className="block w-full rounded-lg bg-zinc-900 px-4 py-3 text-center text-base font-semibold text-white transition hover:bg-zinc-700">Edit Job</Link>
              <button type="button" onClick={() => { setIsVerificationOpen(false); setDeleteError(undefined); setIsDeleteOpen(true); }} className="w-full rounded-lg border border-red-300 bg-white px-4 py-3 text-base font-semibold text-red-700 transition hover:bg-red-50">Delete Job</button>
            </aside>
          </div>
        </div>
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <div className="h-50">
            <DashboardAreaChart labels={trend.map((item) => item.day)} datasets={[{ label: 'Applications', data: trend.map((item) => item.applications), border_color: '#525252', background_color: 'rgba(82,82,82,0.2)' }]} />
          </div>
        </Card>
      </div>
      ...
      <ConfirmationModal
        ...
        message={job.status?.toLowerCase() === 'published' ? 'This job is published and needs an additional verification step before deletion.' : 'This action permanently removes the job post and cannot be undone.'}
