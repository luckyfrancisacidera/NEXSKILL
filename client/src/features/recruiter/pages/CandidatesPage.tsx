...
export const CandidatesPage = () => {
  ...
  return (
    <Card>
      ...
      <Form method="get" ref={filterFormRef} onChange={() => submitFilters()} className="mb-5 mt-4 flex flex-wrap items-end gap-3">
        <SearchField label="Search" name="search" defaultValue={normalizedFilters.search} placeholder="Search name or email" className="min-w-65 flex-1" />
        ...
        <Dropdown
          ...
          options={[
            ...
            ...departments.map((department) => ({ value: department, label: department, accentClassName: 'bg-zinc-100 text-zinc-700' })),
          ]}
          className="min-w-60"
          onChange={submitFilters}
        />

        <JobFilterDropdown jobs={jobs} filters={normalizedFilters} counts={counts} onChange={submitFilters} />
        ...
        <div className="flex flex-wrap items-center gap-2">
          {(stage === 'all' || stage === 'Recommended') && (
            <button
              ...
              onClick={() => queueBulkAction({ action: 'shortlist', status: 'Shortlisted', label: 'Shortlist', title: 'Shortlist candidates', message: `Move ${selectedIdsOnPage.length} selected candidate(s) to Shortlisted stage?`, accent: 'green' })}
          ...
          {stage === 'Shortlisted' && (
            <>
              <button type="button" disabled={selectedIdsOnPage.length === 0 || isSubmittingAction} onClick={() => queueBulkAction({ action: 'remove-shortlist', status: 'Applied', label: 'Remove from shortlist', title: 'Remove from shortlist', message: `Remove shortlist status for ${selectedIdsOnPage.length} selected candidate(s)?`, accent: 'violet' })} className="rounded-xl border border-violet-300 bg-white px-3.5 py-2 text-sm text-violet-700 shadow-sm transition hover:bg-violet-50 disabled:cursor-not-allowed disabled:border-zinc-200 disabled:text-zinc-400">Remove from Shortlist</button>
              <button type="button" disabled={selectedIdsOnPage.length === 0 || isSubmittingAction} onClick={() => queueBulkAction({ action: 'set-interview', status: 'Interview', label: 'Set Interview', title: 'Move to interview', message: `Move ${selectedIdsOnPage.length} selected candidate(s) to Interview stage?`, accent: 'green' })} className="rounded-xl bg-zinc-900 px-3.5 py-2 text-sm text-white shadow-sm disabled:cursor-not-allowed disabled:bg-zinc-300">Set Interview</button>
              <button type="button" disabled={selectedIdsOnPage.length === 0 || isSubmittingAction} onClick={() => queueBulkAction({ action: 'reject', status: 'Rejected', label: 'Reject', title: 'Reject candidates', message: `Reject ${selectedIdsOnPage.length} selected candidate(s)? This cannot be undone.`, accent: 'red' })} className="rounded-xl bg-rose-600 px-3.5 py-2 text-sm text-white shadow-sm transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:bg-rose-200">Reject</button>
            </>
          )}

          {stage === 'Interview' && (
            <>
              <button type="button" disabled={selectedIdsOnPage.length === 0 || isSubmittingAction} onClick={() => queueBulkAction({ action: 'shortlist', status: 'Shortlisted', label: 'Shortlist', title: 'Shortlist interview candidates', message: `Move ${selectedIdsOnPage.length} selected interview candidate(s) back to Shortlisted stage?`, accent: 'violet' })} className="rounded-xl border border-violet-300 bg-white px-3.5 py-2 text-sm text-violet-700 shadow-sm transition hover:bg-violet-50 disabled:cursor-not-allowed disabled:border-zinc-200 disabled:text-zinc-400">Shortlist</button>
              <button type="button" disabled={selectedIdsOnPage.length === 0 || isSubmittingAction} onClick={() => queueBulkAction({ action: 'offer', status: 'Offer', label: 'Give Offer', title: 'Move to offer', message: `Move ${selectedIdsOnPage.length} selected candidate(s) to Offer stage?`, accent: 'green' })} className="rounded-xl bg-zinc-900 px-3.5 py-2 text-sm text-white shadow-sm disabled:cursor-not-allowed disabled:bg-zinc-300">Give Offer</button>
              <button type="button" disabled={selectedIdsOnPage.length === 0 || isSubmittingAction} onClick={() => queueBulkAction({ action: 'reject', status: 'Rejected', label: 'Reject', title: 'Reject candidates', message: `Reject ${selectedIdsOnPage.length} selected candidate(s)? This cannot be undone.`, accent: 'red' })} className="rounded-xl bg-rose-600 px-3.5 py-2 text-sm text-white shadow-sm transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:bg-rose-200">Reject</button>
            </>
          )}
          ...
          {stage === 'Offer' && (
            <>
              <button type="button" disabled={selectedIdsOnPage.length === 0 || isSubmittingAction} onClick={() => queueBulkAction({ action: 'hire', status: 'Hire', label: 'Hire', title: 'Hire candidates', message: `Move ${selectedIdsOnPage.length} selected candidate(s) to Hire stage?`, accent: 'green' })} className="rounded-xl bg-zinc-900 px-3.5 py-2 text-sm text-white shadow-sm disabled:cursor-not-allowed disabled:bg-zinc-300">Hire</button>
              <button type="button" disabled={selectedIdsOnPage.length === 0 || isSubmittingAction} onClick={() => queueBulkAction({ action: 'reject', status: 'Rejected', label: 'Reject', title: 'Reject candidates', message: `Reject ${selectedIdsOnPage.length} selected candidate(s)? This cannot be undone.`, accent: 'red' })} className="rounded-xl bg-rose-600 px-3.5 py-2 text-sm text-white shadow-sm transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:bg-rose-200">Reject</button>
            </>
          )}
          ...
          <thead className="bg-zinc-100 text-zinc-700">
            <tr>
              <th className="w-12 px-3 py-3 text-left">
                <input type="checkbox" aria-label="select all candidates" checked={isAllChecked} onChange={toggleAllRows} className="h-4 w-4 rounded border-zinc-400" />
              </th>
              {['Name', 'Email', 'Job', 'Status', 'Score', 'Applied', 'Actions'].map((col) => (
                <th key={col} className="px-3 py-3 text-left font-semibold">{col}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {candidates.map((candidate, idx) => (
              <tr key={candidate.resume_submission_id} className={`border-t border-zinc-100 ${idx % 2 ? 'bg-zinc-50/60' : 'bg-white'}`}>
                <td className="px-3 py-3">
                  <input type="checkbox" aria-label={`select ${candidate.applicant_name}`} checked={selectedSet.has(candidate.resume_submission_id)} onChange={() => toggleSingleRow(candidate.resume_submission_id)} className="h-4 w-4 rounded border-zinc-400" />
                </td>
                ...
                <td className="px-3 py-3"><span className="rounded-full bg-zinc-200 px-2.5 py-1 text-xs font-medium text-zinc-700">{candidate.submission_status}</span></td>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-zinc-200 pt-4 text-sm">
        <span>Page {pagination.page} of {pagination.totalPages} - {pagination.total} candidates</span>
        <div className="flex gap-2">
          <Link to={`?search=${encodeURIComponent(normalizedFilters.search)}&jobId=${encodeURIComponent(normalizedFilters.jobId)}&department=${encodeURIComponent(normalizedFilters.department)}&recommendedTopPercent=${encodeURIComponent(normalizedFilters.recommendedTopPercent)}&stage=${encodeURIComponent(normalizedFilters.stage)}&pageSize=${encodeURIComponent(normalizedFilters.pageSize)}&page=${Math.max(1, pagination.page - 1)}`} className="rounded border border-zinc-300 px-3 py-1">Prev</Link>
          <Link to={`?search=${encodeURIComponent(normalizedFilters.search)}&jobId=${encodeURIComponent(normalizedFilters.jobId)}&department=${encodeURIComponent(normalizedFilters.department)}&recommendedTopPercent=${encodeURIComponent(normalizedFilters.recommendedTopPercent)}&stage=${encodeURIComponent(normalizedFilters.stage)}&pageSize=${encodeURIComponent(normalizedFilters.pageSize)}&page=${Math.min(pagination.totalPages, pagination.page + 1)}`} className="rounded border border-zinc-300 px-3 py-1">Next</Link>
        </div>
      </div>
