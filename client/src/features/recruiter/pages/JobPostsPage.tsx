    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-zinc-100 text-left">
              <tr>
                {["Title", "Department", "Location", "Type", "Status", "Actions"].map(
                  (col) => (
            ...
            <tbody>
              {jobs.map((job, idx) => {
                ...
                return (
                  <tr
                    key={job.id}
                    className={idx % 2 ? "bg-zinc-50" : "bg-white"}
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-lg border px-3 py-1 text-sm font-medium ${statusAccent.className}`}
                      >
                        {statusAccent.label}
                      </span>
                    </td>
        ...
        <div className="flex items-center justify-between border-t border-zinc-200 p-4 text-sm">
          ...
          <div className="flex gap-2">
            <Link
              to={`/recruiter/job-posts?${new URLSearchParams({
