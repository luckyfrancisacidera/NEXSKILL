      ...
      <Card>
        ...
        <select
            ...
            onChange={(event) => {
              ...
              navigate(`/recruiter/job-posts?${buildQuery({ department, page: "1" })}`);
            }}
            className="rounded-lg border border-zinc-300 px-3 py-2 text-sm"
          >
            ...
            <thead className="bg-zinc-100 text-left">
              <tr>
               {["Title", "Department", "Location", "Type", "Status", "Actions"].map((col) => (
                  <th key={col} className="px-4 py-3 font-medium text-zinc-700">
                    {col}
                  </th>
                ))}
              </tr>
                  ...
                  <tr key={job.id} className={idx % 2 ? "bg-zinc-50" : "bg-white"}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <Link className="rounded border border-zinc-300 px-2 py-1" to={`/recruiter/job-posts/${job.id}`}>
                          <Eye size={16} />
                        </Link>
                        <Link className="rounded border border-zinc-300 px-2 py-1" to={`/recruiter/job-posts/${job.id}/edit`}>
                          <Pencil size={16} />
                        </Link>
                        <button
                          type="button"
                          className="rounded border border-rose-300 px-2 py-1 text-rose-700"
                          onClick={() => openDeleteFlow(job)}
                          disabled={isDeleting}
                        >
                          <Trash2 size={16} />
                        </button>
        ...
        <div className="flex items-center justify-between border-t border-zinc-200 p-4 text-sm">
          <span>
            Page {loaderData.page} of {pageCount}
          </span>
          <div className="flex items-center gap-2">
            <select
              value={String(loaderData.pageSize)}
              className="rounded border border-zinc-300 px-2 py-1 min-w-30 text-sm"
              onChange={(event) => navigate(`/recruiter/job-posts?${buildQuery({ pageSize: event.target.value, page: "1" })}`)}
            >
              {[10, 20, 50].map((size) => (
            ...
            <Link
              to={`/recruiter/job-posts?${buildQuery({ page: String(Math.max(1, loaderData.page - 1)) })}`}
              className="rounded border border-zinc-300 px-3 py-1"
            >
              Prev
            </Link>
            <Link
              to={`/recruiter/job-posts?${buildQuery({ page: String(Math.min(pageCount, loaderData.page + 1)) })}`}
              className="rounded border border-zinc-300 px-3 py-1"
            >
              Next
            </Link>
